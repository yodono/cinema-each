...
export default function GenerosMensalPage() {
  ...
  const resumo: ResumoData | null = useMemo(() => {
    ...
    const filtered = typedData.filter((r) =>
      selectedGeneros.includes(r.nome_genero)
    );
    if (!filtered.length)
      return {
        totalIngressos: 0,
          ...
          <div className="space-y-4 pt-6 p-6">
            ...
            <div className="flex gap-2">
              <Button
                size="sm"
                variant="outline"
                onClick={() => setSelectedGeneros(allGenerosList)}
              >
      ...
      {!q.isLoading && resumo && (
        <div className="grid md:grid-cols-4 gap-6">
          <GlassStat
            ...
            value={numberFormatter(resumo.totalIngressos)}
            icon={Ticket}
            color={PRIMARY}
          />
          <GlassStat
            title="Gênero Mais Assistido"
            value={resumo.topGenero}
            icon={TrendingUp}
            color={SECONDARY}
          />
          <GlassStat
            title="Ingressos do Líder"
            value={numberFormatter(resumo.topValor)}
            icon={Layers}
            color={WARNING}
          />
          <GlassStat
            title="Gêneros Selecionados"
            value={resumo.totalGenerosSelecionados}
            icon={Film}
            color={PRIMARY}
          />
        </div>
        ...
        <div className="p-6">
          ...
          {!q.isLoading && chartData.length > 0 && (
            <div
              style={{
                width: "100%",
                height: chartData.length > 6 ? 600 : 420,
              }}
            >
          ...
          {!q.isLoading && chartData.length === 0 && (
            <p className="text-sm text-muted-foreground">
              Nenhum registro encontrado ou nenhum gênero selecionado para o
              período.
            </p>
          )}
        </div>
